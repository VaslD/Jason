...
public extension JSONEncoder {
    /// 使用当前已配置的 `JSONEncoder` 将 `Encodable` 转换为 ``Jason/Jason``。
    ///
    /// `Encodable` 不能直接转换为 ``Jason/Jason``，因为调用者可能希望更改属性命名规则、时间戳起始日期等编码设置。在配置
    /// `JSONEncoder` 之后，使用此方法将 `Encodable` 转换为 ``Jason/Jason``。
    ///
