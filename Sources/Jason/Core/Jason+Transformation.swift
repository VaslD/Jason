public extension Jason {
    /// 将 ``Jason/Jason`` 映射为任意类型 `T` 的数组。
    ///
    /// > Tip: 此方法将保留空值。如果只希望获取映射后非空的元素，使用 ``compactMap(_:)``。
    ///
    /// 此方法工作原理如下：
    ///
    /// - 对于 ``array(_:)`` 中的每一个元素执行映射，索引为 ``JasonIndex/element(_:)``，返回新元素数组。
    ...
    /// 将 ``Jason/Jason`` 映射为任意类型 `T` 的数组，并在映射过程中丢弃空值。
    ///
    /// 此方法工作原理如下：
    /// 
    /// - 对于 ``array(_:)`` 中的每一个元素执行映射，索引为 ``JasonIndex/element(_:)``，返回非空元素数组。
