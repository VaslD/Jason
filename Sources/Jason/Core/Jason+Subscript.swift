public extension Jason {
    ...
    ///
    /// > Tip:
    /// 当不需要立即获取 ``Jason/Jason`` 内部的值时，建议直接传递 ``Jason/Jason`` 而非检查每一步是否为 ``empty``。由于 ``empty``
    /// 具有 ``Jason/Jason`` 的所有功能，函数和模块之间可以交换 ``Jason/Jason`` 并最终统一处理结果或错误。
    ///
    /// 赋值时，请注意 ``Jason/Jason`` 是关联值枚举而不是类；每次修改都会产生新的 ``Jason/Jason``，因此赋值不会影响其他地方持有的副本。
    ///
    /// > Warning:
    ...
    /// 索引赋值也会**操作无效**。调用此方法保存计算后和获取到的值时请注意 ``Jason/Jason`` 此时的类型，不当操作可能引起非预期的数据丢失。
    ///
    /// > Important:
    /// 此方法**允许**跨层级赋值。如果赋值过程中某层级尚不存在，将会根据索引自动创建对应类型的
    /// ``Jason/Jason``，以继续前往下一层级。``JasonIndex/property(_:)`` (`String`) 索引会导致不存在的层级创建为
    ...
    /// 支持传入层级数组变量调用的快捷方法。
    ///
    /// 此方法重定向至 ``subscript(_:)-7vljl`` 但仅支持取值。
    @inlinable
